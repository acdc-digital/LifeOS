// Activity Bar Component - Left navigation sidebar for LifeOS platform
// /Users/matthewsimon/Projects/LifeOS/LifeOS/app/_components/dashboard/dashActivityBar.tsx
...
export function DashActivityBar({ activePanel, onPanelChange }: ActivityBarProps) {
  ...
  const activityItems: Array<{ id: PanelType; icon: LucideIcon; label: string }> = [
    ...
    { id: "agents-extensions", icon: Bot, label: "Agents & Extensions" },
    { id: "books", icon: BookOpen, label: "Books" },
    { id: "health-fitness", icon: Activity, label: "Health & Fitness" },
  ...
  const handleActivityClick = (id: PanelType) => {
    // Handle notes special case - open both sidebar and tab
    if (id === 'notes') {
    ...
    // Handle books special case - open both sidebar and tab
    if (id === 'books') {
      openSpecialTab('books', 'Books', 'books');
    }
